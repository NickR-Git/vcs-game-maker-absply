...
// localStorage, directly, before ANY of hooks/storage.js's  reactive refs
// are created for these keys (see that file's  comment on why a key's
// initial value is only ever read once, the first time it's accessed) - see
// main.js's  call site, which runs this before the app is mounted, same
...
// ids only ever needed to be unique WITHIN one player's  list before (see
// PlayerEditor.vue's  getMaxId) - two independently-saved projects'
...
// Shifts every sprite_player1_animation_select block's  stored VAR field
// (the animation's position in what USED to be Player 1's  separate list -
// see blocks/sprites.js's  generator, which reads this field as a plain
...
// block with its  unrelated 'VAR' field (e.g. a variable getter plugged
// into this block's  inputs elsewhere in the tree), which must NOT be
// touched here.
